...
public class OrderController {
    ...
    /*구매이력을 조회할  수 있도록 콘트롤러에 지금까지 구현한 로직들을 호출하는 메소드를 만듬*/
    @GetMapping(value = {"/orders", "/orders/{page}"})
    public String orderHist(@PathVariable("page") Optional<Integer> page, Principal principal, Model model){

        Pageable pageable = PageRequest.of(page.isPresent() ? page.get() : 0, 4);//한번에 가지고올 주문의 개수는 4개로 설정
        Page<OrderHistDto> ordersHistDtoList = orderService.getOrderList(principal.getName(), pageable);//현재 로그인한 회원은 이메
        //일과 페이징 객체를 파라미터로 전달해 화면에 전달한 주문 목록 데이터를 리턴 값으로 받음
    ...
    /*주문번호orderId 를 받아 주문 취소 로직을 호출하는 메소드. 비동기 처리*/
